...
tbl_name2 = 'public.tbl_acquisition_user_history' #'public.tbl_acquisition_partner_reactivation_config'
...
def expired(_id, offset, limit):
    ...
    q = f"SELECT id FROM {tbl_name2} LIMIT {limit} OFFSET {offset}"
    res = execute(q).fetchall()
    for row in res:
...
def scheduler(pid, offset, limit=None, n_workers=None, waiters_list=None):
    ...
    sub_offset = offset
    sub_limit = LIMIT #limit or ceil(LIMIT / n_workers)
    ...
    if len(statuses) > 0:
        with ThreadPoolExecutor(max_workers=n_workers) as executor:
            for idx, status in enumerate(statuses, 1):
                ...
                print(f'{_id} - Started Offset: {sub_offset} Limit: {sub_limit}')
                if status == 'Initiated':
            ...
            if len(waiters_list) > 0:
                ...
                for idx, waiter in enumerate(as_completed(waiters_list.values())):
                    ...
                    print(f'Completed: {status} Offset: {sub_offset} To: {sub_limit}')
                return True
